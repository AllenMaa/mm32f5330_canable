...
void USART_InitSync(USART_Type * USARTx, USART_InitSync_Type * init)
{
    ...
    uint32_t cr2 = USARTx->CR2 & ~( USART_CR2_SWAP_MASK | USART_CR2_STOP_MASK | USART_CR2_CLKEN_MASK
                                  | USART_CR2_CPOL_MASK | USART_CR2_CPHA_MASK | USART_CR2_LINEN_MASK);
    ...
    cr2 |= (  USART_CR2_SWAP(init->EnableSwapTxRxXferSignal) | USART_CR2_STOP(init->StopBits) 
           ...
           | USART_CR2_CLKEN_MASK);   //USART_CR2_CLKEN(init->EnableClockOutput) );
    USARTx->CR2 = cr2;

    uint32_t cr3 = USARTx->CR3 & ~( USART_CR3_TXTOG_MASK |  USART_CR3_RXTOG_MASK | USART_CR3_SCEN_MASK
                                  | USART_CR3_HDSEL_MASK | USART_CR3_IRDAEN_MASK);
...
void USART_InitAsync(USART_Type * USARTx, USART_InitAsync_Type * init)
{
    ...
    uint32_t cr2 = USARTx->CR2 & ~(  USART_CR2_SWAP_MASK | USART_CR2_STOP_MASK
                                   | USART_CR2_CLKEN_MASK | USART_CR2_LINEN_MASK);

    /* SwapTxRxXferSignal, StopBits. */
    cr2 |= ( USART_CR2_SWAP(init->EnableSwapTxRxXferSignal) | USART_CR2_STOP(init->StopBits) );
    USARTx->CR2 = cr2;

    uint32_t cr3 = USARTx->CR3 & ~( USART_CR3_TXTOG_MASK |  USART_CR3_RXTOG_MASK
                                  | USART_CR3_SCEN_MASK | USART_CR3_HDSEL_MASK | USART_CR3_IRDAEN_MASK);
...
void USART_EnableDMA(USART_Type * USARTx, bool enable)
{
    if (enable)
    {
        USARTx->CR3 |= USART_CR3_DMAMODE_MASK;
    }
    else
    {
        USARTx->CR3 &= ~USART_CR3_DMAMODE_MASK;
