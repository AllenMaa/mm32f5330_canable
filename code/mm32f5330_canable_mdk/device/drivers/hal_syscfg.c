...
#define SYSCFG_FLXCANMODE_MASK (0xFFFFFFFF)
#define SYSCFG_FLXCANMODE(x)   (((uint32_t)((uint32_t)(x)) ) & SYSCFG_FLXCANMODE_MASK)

#define SYSCFG_DMARemap_MASK (0xFFFFFFFF)
#define SYSCFG_DMARemap(x)   (((uint32_t)((uint32_t)(x)) ) & SYSCFG_DMARemap_MASK)


void SYSCFG_SetFLEXCANMode(SYSCFG_FLEXCANMode_Type mode)
{
    SYSCFG->CFGR1 = ( SYSCFG->CFGR1 & ~ SYSCFG_FLXCANMODE_MASK )
                 | SYSCFG_FLXCANMODE(mode);
}
...
void SYSCFG_SetExtIntMux(SYSCFG_EXTIPort_Type port, SYSCFG_EXTILine_Type line)
{
    ...
    {
        SYSCFG->EXTICR2 = ( SYSCFG->EXTICR2 & ~ SYSCFG_EXTICR_MASK(line) )
                        | ( SYSCFG_EXTICR(port, line) );
    }
    else if ( line < SYSCFG_EXTILine_12 )
    {
        SYSCFG->EXTICR3 = ( SYSCFG->EXTICR3 & ~ SYSCFG_EXTICR_MASK(line) )
                        | ( SYSCFG_EXTICR(port, line) );
    }
    else
    {
        SYSCFG->EXTICR4 = ( SYSCFG->EXTICR4 & ~ SYSCFG_EXTICR_MASK(line) )
                        | ( SYSCFG_EXTICR(port, line) );
    }
}
...
void SYSCFG_SetDMARemap(uint32_t remap)
{
    SYSCFG->DMARMP =  (SYSCFG->DMARMP & ~SYSCFG_DMARemap_MASK) | SYSCFG_DMARemap(remap);
}
